# Model Card: physionet_eegbci_medium_fbcsp_logreg_csp8

## Intended Use
Offline event-locked motor imagery task-label classification.

## Not Intended Use
Not for thought reading, clinical diagnosis, unrestricted mental-state inference, or direct closed-loop control.

## Limitations
Small public EEG subset; CSP is fit inside train splits. Metrics are benchmark evidence only and do not establish clinical validity.

## Metrics
```json
{
  "task_type": "classification",
  "accuracy": 0.5212121212121212,
  "balanced_accuracy": 0.520276226858654,
  "macro_f1": 0.5090210554069833,
  "weighted_f1": 0.5094899425533347,
  "per_class": {
    "LEFT_HAND_IMAGERY": {
      "precision": 0.5185185185185185,
      "recall": 0.6746987951807228,
      "f1-score": 0.5863874345549738,
      "support": 83.0
    },
    "RIGHT_HAND_IMAGERY": {
      "precision": 0.5263157894736842,
      "recall": 0.36585365853658536,
      "f1-score": 0.4316546762589928,
      "support": 82.0
    },
    "accuracy": 0.5212121212121212,
    "macro avg": {
      "precision": 0.5224171539961013,
      "recall": 0.520276226858654,
      "f1-score": 0.5090210554069833,
      "support": 165.0
    },
    "weighted avg": {
      "precision": 0.5223935259022978,
      "recall": 0.5212121212121212,
      "f1-score": 0.5094899425533347,
      "support": 165.0
    }
  },
  "confusion_matrix": [
    [
      56,
      27
    ],
    [
      52,
      30
    ]
  ],
  "class_labels": [
    "LEFT_HAND_IMAGERY",
    "RIGHT_HAND_IMAGERY"
  ],
  "calibration": {
    "available": true,
    "ece": 0.3177166196714878,
    "mce": 0.5025348918796315,
    "reliability_bins": [
      {
        "lo": 0.0,
        "hi": 0.1,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.1,
        "hi": 0.2,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.2,
        "hi": 0.30000000000000004,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.30000000000000004,
        "hi": 0.4,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.4,
        "hi": 0.5,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.5,
        "hi": 0.6,
        "count": 24,
        "accuracy": 0.375,
        "confidence": 0.5432265073050008
      },
      {
        "lo": 0.6000000000000001,
        "hi": 0.7000000000000001,
        "count": 13,
        "accuracy": 0.5384615384615384,
        "confidence": 0.6503863665219799
      },
      {
        "lo": 0.7000000000000001,
        "hi": 0.8,
        "count": 14,
        "accuracy": 0.5714285714285714,
        "confidence": 0.7376207368649922
      },
      {
        "lo": 0.8,
        "hi": 0.9,
        "count": 32,
        "accuracy": 0.34375,
        "confidence": 0.8462848918796315
      },
      {
        "lo": 0.9,
        "hi": 1.0,
        "count": 82,
        "accuracy": 0.6219512195121951,
        "confidence": 0.9697923957247759
      }
    ],
    "brier_score": 0.3478440657714867
  },
  "auroc": 0.5841904202174553,
  "split_strategy": "group_run",
  "split_warnings": [],
  "latency": {
    "mean_inference_latency_ms": 6.951292278752204,
    "p95_inference_latency_ms": 7.273074000477209,
    "p99_inference_latency_ms": 7.453914519355748,
    "throughput_samples_per_sec": 143.85814319111117
  },
  "per_subject": {
    "S001": {
      "task_type": "classification",
      "accuracy": 0.7333333333333333,
      "balanced_accuracy": 0.7321428571428572,
      "macro_f1": 0.7321428571428572,
      "weighted_f1": 0.7333333333333333,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.7142857142857143,
          "recall": 0.7142857142857143,
          "f1-score": 0.7142857142857143,
          "support": 7.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.75,
          "recall": 0.75,
          "f1-score": 0.75,
          "support": 8.0
        },
        "accuracy": 0.7333333333333333,
        "macro avg": {
          "precision": 0.7321428571428572,
          "recall": 0.7321428571428572,
          "f1-score": 0.7321428571428572,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.7333333333333333,
          "recall": 0.7333333333333333,
          "f1-score": 0.7333333333333333,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          5,
          2
        ],
        [
          2,
          6
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S002": {
      "task_type": "classification",
      "accuracy": 0.6666666666666666,
      "balanced_accuracy": 0.6875,
      "macro_f1": 0.6411483253588517,
      "weighted_f1": 0.634768740031898,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5833333333333334,
          "recall": 1.0,
          "f1-score": 0.7368421052631579,
          "support": 7.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 1.0,
          "recall": 0.375,
          "f1-score": 0.5454545454545454,
          "support": 8.0
        },
        "accuracy": 0.6666666666666666,
        "macro avg": {
          "precision": 0.7916666666666667,
          "recall": 0.6875,
          "f1-score": 0.6411483253588517,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.8055555555555556,
          "recall": 0.6666666666666666,
          "f1-score": 0.634768740031898,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          7,
          0
        ],
        [
          5,
          3
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S003": {
      "task_type": "classification",
      "accuracy": 0.3333333333333333,
      "balanced_accuracy": 0.3482142857142857,
      "macro_f1": 0.3055555555555555,
      "weighted_f1": 0.2962962962962962,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.36363636363636365,
          "recall": 0.5714285714285714,
          "f1-score": 0.4444444444444444,
          "support": 7.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.25,
          "recall": 0.125,
          "f1-score": 0.16666666666666666,
          "support": 8.0
        },
        "accuracy": 0.3333333333333333,
        "macro avg": {
          "precision": 0.3068181818181818,
          "recall": 0.3482142857142857,
          "f1-score": 0.3055555555555555,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.303030303030303,
          "recall": 0.3333333333333333,
          "f1-score": 0.2962962962962962,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          4,
          3
        ],
        [
          7,
          1
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S004": {
      "task_type": "classification",
      "accuracy": 0.4,
      "balanced_accuracy": 0.41964285714285715,
      "macro_f1": 0.354066985645933,
      "weighted_f1": 0.34258373205741627,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.3333333333333333,
          "recall": 0.125,
          "f1-score": 0.18181818181818182,
          "support": 8.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.4166666666666667,
          "recall": 0.7142857142857143,
          "f1-score": 0.5263157894736842,
          "support": 7.0
        },
        "accuracy": 0.4,
        "macro avg": {
          "precision": 0.375,
          "recall": 0.41964285714285715,
          "f1-score": 0.354066985645933,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.3722222222222223,
          "recall": 0.4,
          "f1-score": 0.34258373205741627,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          1,
          7
        ],
        [
          2,
          5
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S005": {
      "task_type": "classification",
      "accuracy": 0.4,
      "balanced_accuracy": 0.375,
      "macro_f1": 0.2857142857142857,
      "weighted_f1": 0.30476190476190473,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.0,
          "recall": 0.0,
          "f1-score": 0.0,
          "support": 7.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.46153846153846156,
          "recall": 0.75,
          "f1-score": 0.5714285714285714,
          "support": 8.0
        },
        "accuracy": 0.4,
        "macro avg": {
          "precision": 0.23076923076923078,
          "recall": 0.375,
          "f1-score": 0.2857142857142857,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.24615384615384617,
          "recall": 0.4,
          "f1-score": 0.30476190476190473,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          0,
          7
        ],
        [
          2,
          6
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S006": {
      "task_type": "classification",
      "accuracy": 0.43333333333333335,
      "balanced_accuracy": 0.4151785714285714,
      "macro_f1": 0.3772893772893773,
      "weighted_f1": 0.38974358974358975,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.4782608695652174,
          "recall": 0.6875,
          "f1-score": 0.5641025641025641,
          "support": 16.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.2857142857142857,
          "recall": 0.14285714285714285,
          "f1-score": 0.19047619047619047,
          "support": 14.0
        },
        "accuracy": 0.43333333333333335,
        "macro avg": {
          "precision": 0.38198757763975155,
          "recall": 0.4151785714285714,
          "f1-score": 0.3772893772893773,
          "support": 30.0
        },
        "weighted avg": {
          "precision": 0.3884057971014493,
          "recall": 0.43333333333333335,
          "f1-score": 0.38974358974358975,
          "support": 30.0
        }
      },
      "confusion_matrix": [
        [
          11,
          5
        ],
        [
          12,
          2
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S008": {
      "task_type": "classification",
      "accuracy": 0.6666666666666666,
      "balanced_accuracy": 0.6875,
      "macro_f1": 0.6411483253588517,
      "weighted_f1": 0.634768740031898,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5833333333333334,
          "recall": 1.0,
          "f1-score": 0.7368421052631579,
          "support": 7.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 1.0,
          "recall": 0.375,
          "f1-score": 0.5454545454545454,
          "support": 8.0
        },
        "accuracy": 0.6666666666666666,
        "macro avg": {
          "precision": 0.7916666666666667,
          "recall": 0.6875,
          "f1-score": 0.6411483253588517,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.8055555555555556,
          "recall": 0.6666666666666666,
          "f1-score": 0.634768740031898,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          7,
          0
        ],
        [
          5,
          3
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S009": {
      "task_type": "classification",
      "accuracy": 0.5333333333333333,
      "balanced_accuracy": 0.5,
      "macro_f1": 0.34782608695652173,
      "weighted_f1": 0.3710144927536232,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5333333333333333,
          "recall": 1.0,
          "f1-score": 0.6956521739130435,
          "support": 8.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.0,
          "recall": 0.0,
          "f1-score": 0.0,
          "support": 7.0
        },
        "accuracy": 0.5333333333333333,
        "macro avg": {
          "precision": 0.26666666666666666,
          "recall": 0.5,
          "f1-score": 0.34782608695652173,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.28444444444444444,
          "recall": 0.5333333333333333,
          "f1-score": 0.3710144927536232,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          8,
          0
        ],
        [
          7,
          0
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S010": {
      "task_type": "classification",
      "accuracy": 0.5666666666666667,
      "balanced_accuracy": 0.5491071428571428,
      "macro_f1": 0.5238095238095237,
      "weighted_f1": 0.5333333333333333,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5652173913043478,
          "recall": 0.8125,
          "f1-score": 0.6666666666666666,
          "support": 16.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.5714285714285714,
          "recall": 0.2857142857142857,
          "f1-score": 0.38095238095238093,
          "support": 14.0
        },
        "accuracy": 0.5666666666666667,
        "macro avg": {
          "precision": 0.5683229813664596,
          "recall": 0.5491071428571428,
          "f1-score": 0.5238095238095237,
          "support": 30.0
        },
        "weighted avg": {
          "precision": 0.5681159420289854,
          "recall": 0.5666666666666667,
          "f1-score": 0.5333333333333333,
          "support": 30.0
        }
      },
      "confusion_matrix": [
        [
          13,
          3
        ],
        [
          10,
          4
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    }
  },
  "per_run": {
    "12": {
      "task_type": "classification",
      "accuracy": 0.5333333333333333,
      "balanced_accuracy": 0.532114624505929,
      "macro_f1": 0.53125,
      "weighted_f1": 0.5319444444444444,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.54,
          "recall": 0.5869565217391305,
          "f1-score": 0.5625,
          "support": 46.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.525,
          "recall": 0.4772727272727273,
          "f1-score": 0.5,
          "support": 44.0
        },
        "accuracy": 0.5333333333333333,
        "macro avg": {
          "precision": 0.5325,
          "recall": 0.532114624505929,
          "f1-score": 0.53125,
          "support": 90.0
        },
        "weighted avg": {
          "precision": 0.5326666666666667,
          "recall": 0.5333333333333333,
          "f1-score": 0.5319444444444444,
          "support": 90.0
        }
      },
      "confusion_matrix": [
        [
          27,
          19
        ],
        [
          23,
          21
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "4": {
      "task_type": "classification",
      "accuracy": 0.6,
      "balanced_accuracy": 0.6,
      "macro_f1": 0.5238095238095238,
      "weighted_f1": 0.5238095238095238,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5555555555555556,
          "recall": 1.0,
          "f1-score": 0.7142857142857143,
          "support": 15.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 1.0,
          "recall": 0.2,
          "f1-score": 0.3333333333333333,
          "support": 15.0
        },
        "accuracy": 0.6,
        "macro avg": {
          "precision": 0.7777777777777778,
          "recall": 0.6,
          "f1-score": 0.5238095238095238,
          "support": 30.0
        },
        "weighted avg": {
          "precision": 0.7777777777777779,
          "recall": 0.6,
          "f1-score": 0.5238095238095238,
          "support": 30.0
        }
      },
      "confusion_matrix": [
        [
          15,
          0
        ],
        [
          12,
          3
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "8": {
      "task_type": "classification",
      "accuracy": 0.4444444444444444,
      "balanced_accuracy": 0.4486166007905138,
      "macro_f1": 0.4263131055583886,
      "weighted_f1": 0.4240466881976316,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.45161290322580644,
          "recall": 0.6363636363636364,
          "f1-score": 0.5283018867924528,
          "support": 22.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.42857142857142855,
          "recall": 0.2608695652173913,
          "f1-score": 0.32432432432432434,
          "support": 23.0
        },
        "accuracy": 0.4444444444444444,
        "macro avg": {
          "precision": 0.4400921658986175,
          "recall": 0.4486166007905138,
          "f1-score": 0.4263131055583886,
          "support": 45.0
        },
        "weighted avg": {
          "precision": 0.43983614951356886,
          "recall": 0.4444444444444444,
          "f1-score": 0.4240466881976316,
          "support": 45.0
        }
      },
      "confusion_matrix": [
        [
          14,
          8
        ],
        [
          17,
          6
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    }
  },
  "bootstrap_confidence_intervals": {
    "available": true,
    "n_bootstrap": 500,
    "seed": 42,
    "accuracy": {
      "mean": 0.52,
      "lower_95": 0.4485,
      "upper_95": 0.5939
    },
    "balanced_accuracy": {
      "mean": 0.5195,
      "lower_95": 0.4529,
      "upper_95": 0.5931
    },
    "macro_f1": {
      "mean": 0.5064,
      "lower_95": 0.4347,
      "upper_95": 0.5857
    }
  }
}
```