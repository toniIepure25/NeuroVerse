# Model Card: physionet_eegbci_medium_fbcsp_lda_csp6

## Intended Use
Offline event-locked motor imagery task-label classification.

## Not Intended Use
Not for thought reading, clinical diagnosis, unrestricted mental-state inference, or direct closed-loop control.

## Limitations
Small public EEG subset; CSP is fit inside train splits. Metrics are benchmark evidence only and do not establish clinical validity.

## Metrics
```json
{
  "task_type": "classification",
  "accuracy": 0.5212121212121212,
  "balanced_accuracy": 0.52101087275933,
  "macro_f1": 0.5205781749972416,
  "weighted_f1": 0.5206838326997215,
  "per_class": {
    "LEFT_HAND_IMAGERY": {
      "precision": 0.5227272727272727,
      "recall": 0.5542168674698795,
      "f1-score": 0.5380116959064327,
      "support": 83.0
    },
    "RIGHT_HAND_IMAGERY": {
      "precision": 0.5194805194805194,
      "recall": 0.4878048780487805,
      "f1-score": 0.5031446540880503,
      "support": 82.0
    },
    "accuracy": 0.5212121212121212,
    "macro avg": {
      "precision": 0.5211038961038961,
      "recall": 0.52101087275933,
      "f1-score": 0.5205781749972416,
      "support": 165.0
    },
    "weighted avg": {
      "precision": 0.5211137347500984,
      "recall": 0.5212121212121212,
      "f1-score": 0.5206838326997215,
      "support": 165.0
    }
  },
  "confusion_matrix": [
    [
      46,
      37
    ],
    [
      42,
      40
    ]
  ],
  "class_labels": [
    "LEFT_HAND_IMAGERY",
    "RIGHT_HAND_IMAGERY"
  ],
  "calibration": {
    "available": true,
    "ece": 0.30740283718542244,
    "mce": 0.36574052968753035,
    "reliability_bins": [
      {
        "lo": 0.0,
        "hi": 0.1,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.1,
        "hi": 0.2,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.2,
        "hi": 0.30000000000000004,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.30000000000000004,
        "hi": 0.4,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.4,
        "hi": 0.5,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.5,
        "hi": 0.6,
        "count": 14,
        "accuracy": 0.35714285714285715,
        "confidence": 0.5566685950189856
      },
      {
        "lo": 0.6000000000000001,
        "hi": 0.7000000000000001,
        "count": 20,
        "accuracy": 0.4,
        "confidence": 0.6627570199553239
      },
      {
        "lo": 0.7000000000000001,
        "hi": 0.8,
        "count": 28,
        "accuracy": 0.5714285714285714,
        "confidence": 0.7513923707984725
      },
      {
        "lo": 0.8,
        "hi": 0.9,
        "count": 37,
        "accuracy": 0.4864864864864865,
        "confidence": 0.8512190828240052
      },
      {
        "lo": 0.9,
        "hi": 1.0,
        "count": 66,
        "accuracy": 0.5909090909090909,
        "confidence": 0.9566496205966213
      }
    ],
    "brier_score": 0.3437350846942807
  },
  "auroc": 0.5612694681163679,
  "split_strategy": "group_run",
  "split_warnings": [],
  "latency": {
    "mean_inference_latency_ms": 6.876790830320908,
    "p95_inference_latency_ms": 7.251521200305433,
    "p99_inference_latency_ms": 7.582730599242494,
    "throughput_samples_per_sec": 145.41666667987553
  },
  "per_subject": {
    "S001": {
      "task_type": "classification",
      "accuracy": 0.6666666666666666,
      "balanced_accuracy": 0.6696428571428572,
      "macro_f1": 0.6666666666666666,
      "weighted_f1": 0.6666666666666666,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.625,
          "recall": 0.7142857142857143,
          "f1-score": 0.6666666666666666,
          "support": 7.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.7142857142857143,
          "recall": 0.625,
          "f1-score": 0.6666666666666666,
          "support": 8.0
        },
        "accuracy": 0.6666666666666666,
        "macro avg": {
          "precision": 0.6696428571428572,
          "recall": 0.6696428571428572,
          "f1-score": 0.6666666666666666,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.6726190476190477,
          "recall": 0.6666666666666666,
          "f1-score": 0.6666666666666666,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          5,
          2
        ],
        [
          3,
          5
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S002": {
      "task_type": "classification",
      "accuracy": 0.7333333333333333,
      "balanced_accuracy": 0.75,
      "macro_f1": 0.7222222222222222,
      "weighted_f1": 0.7185185185185186,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.6363636363636364,
          "recall": 1.0,
          "f1-score": 0.7777777777777778,
          "support": 7.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 1.0,
          "recall": 0.5,
          "f1-score": 0.6666666666666666,
          "support": 8.0
        },
        "accuracy": 0.7333333333333333,
        "macro avg": {
          "precision": 0.8181818181818181,
          "recall": 0.75,
          "f1-score": 0.7222222222222222,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.8303030303030302,
          "recall": 0.7333333333333333,
          "f1-score": 0.7185185185185186,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          7,
          0
        ],
        [
          4,
          4
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S003": {
      "task_type": "classification",
      "accuracy": 0.4,
      "balanced_accuracy": 0.4107142857142857,
      "macro_f1": 0.3891402714932127,
      "weighted_f1": 0.38371040723981903,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.4,
          "recall": 0.5714285714285714,
          "f1-score": 0.47058823529411764,
          "support": 7.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.4,
          "recall": 0.25,
          "f1-score": 0.3076923076923077,
          "support": 8.0
        },
        "accuracy": 0.4,
        "macro avg": {
          "precision": 0.4,
          "recall": 0.4107142857142857,
          "f1-score": 0.3891402714932127,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.4,
          "recall": 0.4,
          "f1-score": 0.38371040723981903,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          4,
          3
        ],
        [
          6,
          2
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S004": {
      "task_type": "classification",
      "accuracy": 0.5333333333333333,
      "balanced_accuracy": 0.5267857142857143,
      "macro_f1": 0.5248868778280543,
      "weighted_f1": 0.5291101055806938,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5555555555555556,
          "recall": 0.625,
          "f1-score": 0.5882352941176471,
          "support": 8.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.5,
          "recall": 0.42857142857142855,
          "f1-score": 0.46153846153846156,
          "support": 7.0
        },
        "accuracy": 0.5333333333333333,
        "macro avg": {
          "precision": 0.5277777777777778,
          "recall": 0.5267857142857143,
          "f1-score": 0.5248868778280543,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.5296296296296297,
          "recall": 0.5333333333333333,
          "f1-score": 0.5291101055806938,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          5,
          3
        ],
        [
          4,
          3
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S005": {
      "task_type": "classification",
      "accuracy": 0.4,
      "balanced_accuracy": 0.375,
      "macro_f1": 0.2857142857142857,
      "weighted_f1": 0.30476190476190473,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.0,
          "recall": 0.0,
          "f1-score": 0.0,
          "support": 7.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.46153846153846156,
          "recall": 0.75,
          "f1-score": 0.5714285714285714,
          "support": 8.0
        },
        "accuracy": 0.4,
        "macro avg": {
          "precision": 0.23076923076923078,
          "recall": 0.375,
          "f1-score": 0.2857142857142857,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.24615384615384617,
          "recall": 0.4,
          "f1-score": 0.30476190476190473,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          0,
          7
        ],
        [
          2,
          6
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S006": {
      "task_type": "classification",
      "accuracy": 0.5333333333333333,
      "balanced_accuracy": 0.5446428571428572,
      "macro_f1": 0.5248868778280543,
      "weighted_f1": 0.5206636500754148,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.6,
          "recall": 0.375,
          "f1-score": 0.46153846153846156,
          "support": 16.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.5,
          "recall": 0.7142857142857143,
          "f1-score": 0.5882352941176471,
          "support": 14.0
        },
        "accuracy": 0.5333333333333333,
        "macro avg": {
          "precision": 0.55,
          "recall": 0.5446428571428572,
          "f1-score": 0.5248868778280543,
          "support": 30.0
        },
        "weighted avg": {
          "precision": 0.5533333333333333,
          "recall": 0.5333333333333333,
          "f1-score": 0.5206636500754148,
          "support": 30.0
        }
      },
      "confusion_matrix": [
        [
          6,
          10
        ],
        [
          4,
          10
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S008": {
      "task_type": "classification",
      "accuracy": 0.6666666666666666,
      "balanced_accuracy": 0.6785714285714286,
      "macro_f1": 0.660633484162896,
      "weighted_f1": 0.6576168929110106,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.6,
          "recall": 0.8571428571428571,
          "f1-score": 0.7058823529411765,
          "support": 7.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.8,
          "recall": 0.5,
          "f1-score": 0.6153846153846154,
          "support": 8.0
        },
        "accuracy": 0.6666666666666666,
        "macro avg": {
          "precision": 0.7,
          "recall": 0.6785714285714286,
          "f1-score": 0.660633484162896,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.7066666666666668,
          "recall": 0.6666666666666666,
          "f1-score": 0.6576168929110106,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          6,
          1
        ],
        [
          4,
          4
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S009": {
      "task_type": "classification",
      "accuracy": 0.4,
      "balanced_accuracy": 0.3839285714285714,
      "macro_f1": 0.354066985645933,
      "weighted_f1": 0.36555023923444974,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.45454545454545453,
          "recall": 0.625,
          "f1-score": 0.5263157894736842,
          "support": 8.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.25,
          "recall": 0.14285714285714285,
          "f1-score": 0.18181818181818182,
          "support": 7.0
        },
        "accuracy": 0.4,
        "macro avg": {
          "precision": 0.3522727272727273,
          "recall": 0.3839285714285714,
          "f1-score": 0.354066985645933,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.3590909090909091,
          "recall": 0.4,
          "f1-score": 0.36555023923444974,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          5,
          3
        ],
        [
          6,
          1
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S010": {
      "task_type": "classification",
      "accuracy": 0.43333333333333335,
      "balanced_accuracy": 0.4285714285714286,
      "macro_f1": 0.4276094276094276,
      "weighted_f1": 0.4314253647586981,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.47058823529411764,
          "recall": 0.5,
          "f1-score": 0.48484848484848486,
          "support": 16.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.38461538461538464,
          "recall": 0.35714285714285715,
          "f1-score": 0.37037037037037035,
          "support": 14.0
        },
        "accuracy": 0.43333333333333335,
        "macro avg": {
          "precision": 0.42760180995475117,
          "recall": 0.4285714285714286,
          "f1-score": 0.4276094276094276,
          "support": 30.0
        },
        "weighted avg": {
          "precision": 0.43046757164404226,
          "recall": 0.43333333333333335,
          "f1-score": 0.4314253647586981,
          "support": 30.0
        }
      },
      "confusion_matrix": [
        [
          8,
          8
        ],
        [
          9,
          5
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    }
  },
  "per_run": {
    "12": {
      "task_type": "classification",
      "accuracy": 0.5,
      "balanced_accuracy": 0.5014822134387352,
      "macro_f1": 0.4984520123839009,
      "weighted_f1": 0.49783281733746126,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5128205128205128,
          "recall": 0.43478260869565216,
          "f1-score": 0.47058823529411764,
          "support": 46.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.49019607843137253,
          "recall": 0.5681818181818182,
          "f1-score": 0.5263157894736842,
          "support": 44.0
        },
        "accuracy": 0.5,
        "macro avg": {
          "precision": 0.5015082956259427,
          "recall": 0.5014822134387352,
          "f1-score": 0.4984520123839009,
          "support": 90.0
        },
        "weighted avg": {
          "precision": 0.5017596782302665,
          "recall": 0.5,
          "f1-score": 0.49783281733746126,
          "support": 90.0
        }
      },
      "confusion_matrix": [
        [
          20,
          26
        ],
        [
          19,
          25
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "4": {
      "task_type": "classification",
      "accuracy": 0.6,
      "balanced_accuracy": 0.6,
      "macro_f1": 0.569377990430622,
      "weighted_f1": 0.5693779904306221,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5652173913043478,
          "recall": 0.8666666666666667,
          "f1-score": 0.6842105263157895,
          "support": 15.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.7142857142857143,
          "recall": 0.3333333333333333,
          "f1-score": 0.45454545454545453,
          "support": 15.0
        },
        "accuracy": 0.6,
        "macro avg": {
          "precision": 0.639751552795031,
          "recall": 0.6,
          "f1-score": 0.569377990430622,
          "support": 30.0
        },
        "weighted avg": {
          "precision": 0.6397515527950312,
          "recall": 0.6,
          "f1-score": 0.5693779904306221,
          "support": 30.0
        }
      },
      "confusion_matrix": [
        [
          13,
          2
        ],
        [
          10,
          5
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "8": {
      "task_type": "classification",
      "accuracy": 0.5111111111111111,
      "balanced_accuracy": 0.5128458498023716,
      "macro_f1": 0.5089285714285714,
      "weighted_f1": 0.5082010582010582,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5,
          "recall": 0.5909090909090909,
          "f1-score": 0.5416666666666666,
          "support": 22.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.5263157894736842,
          "recall": 0.43478260869565216,
          "f1-score": 0.47619047619047616,
          "support": 23.0
        },
        "accuracy": 0.5111111111111111,
        "macro avg": {
          "precision": 0.513157894736842,
          "recall": 0.5128458498023716,
          "f1-score": 0.5089285714285714,
          "support": 45.0
        },
        "weighted avg": {
          "precision": 0.5134502923976608,
          "recall": 0.5111111111111111,
          "f1-score": 0.5082010582010582,
          "support": 45.0
        }
      },
      "confusion_matrix": [
        [
          13,
          9
        ],
        [
          13,
          10
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    }
  },
  "bootstrap_confidence_intervals": {
    "available": true,
    "n_bootstrap": 500,
    "seed": 42,
    "accuracy": {
      "mean": 0.5225,
      "lower_95": 0.4424,
      "upper_95": 0.6
    },
    "balanced_accuracy": {
      "mean": 0.5225,
      "lower_95": 0.4483,
      "upper_95": 0.5996
    },
    "macro_f1": {
      "mean": 0.5203,
      "lower_95": 0.439,
      "upper_95": 0.5985
    }
  }
}
```