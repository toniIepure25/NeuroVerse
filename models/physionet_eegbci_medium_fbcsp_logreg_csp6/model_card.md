# Model Card: physionet_eegbci_medium_fbcsp_logreg_csp6

## Intended Use
Offline event-locked motor imagery task-label classification.

## Not Intended Use
Not for thought reading, clinical diagnosis, unrestricted mental-state inference, or direct closed-loop control.

## Limitations
Small public EEG subset; CSP is fit inside train splits. Metrics are benchmark evidence only and do not establish clinical validity.

## Metrics
```json
{
  "task_type": "classification",
  "accuracy": 0.5333333333333333,
  "balanced_accuracy": 0.5325448133999413,
  "macro_f1": 0.5248868778280543,
  "weighted_f1": 0.5252708076237489,
  "per_class": {
    "LEFT_HAND_IMAGERY": {
      "precision": 0.5288461538461539,
      "recall": 0.6626506024096386,
      "f1-score": 0.5882352941176471,
      "support": 83.0
    },
    "RIGHT_HAND_IMAGERY": {
      "precision": 0.5409836065573771,
      "recall": 0.4024390243902439,
      "f1-score": 0.46153846153846156,
      "support": 82.0
    },
    "accuracy": 0.5333333333333333,
    "macro avg": {
      "precision": 0.5349148802017655,
      "recall": 0.5325448133999413,
      "f1-score": 0.5248868778280543,
      "support": 165.0
    },
    "weighted avg": {
      "precision": 0.5348781000420345,
      "recall": 0.5333333333333333,
      "f1-score": 0.5252708076237489,
      "support": 165.0
    }
  },
  "confusion_matrix": [
    [
      55,
      28
    ],
    [
      49,
      33
    ]
  ],
  "class_labels": [
    "LEFT_HAND_IMAGERY",
    "RIGHT_HAND_IMAGERY"
  ],
  "calibration": {
    "available": true,
    "ece": 0.29670117002175245,
    "mce": 0.38431218979942205,
    "reliability_bins": [
      {
        "lo": 0.0,
        "hi": 0.1,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.1,
        "hi": 0.2,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.2,
        "hi": 0.30000000000000004,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.30000000000000004,
        "hi": 0.4,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.4,
        "hi": 0.5,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.5,
        "hi": 0.6,
        "count": 14,
        "accuracy": 0.2857142857142857,
        "confidence": 0.5527850269488194
      },
      {
        "lo": 0.6000000000000001,
        "hi": 0.7000000000000001,
        "count": 24,
        "accuracy": 0.5833333333333334,
        "confidence": 0.6465685412424725
      },
      {
        "lo": 0.7000000000000001,
        "hi": 0.8,
        "count": 24,
        "accuracy": 0.5,
        "confidence": 0.7572423460662187
      },
      {
        "lo": 0.8,
        "hi": 0.9,
        "count": 30,
        "accuracy": 0.5333333333333333,
        "confidence": 0.8490150508513093
      },
      {
        "lo": 0.9,
        "hi": 1.0,
        "count": 73,
        "accuracy": 0.5753424657534246,
        "confidence": 0.9596546555528467
      }
    ],
    "brier_score": 0.33824020289177825
  },
  "auroc": 0.5714075815456949,
  "split_strategy": "group_run",
  "split_warnings": [],
  "latency": {
    "mean_inference_latency_ms": 7.7609291455287766,
    "p95_inference_latency_ms": 8.905698599119205,
    "p99_inference_latency_ms": 9.555055679520589,
    "throughput_samples_per_sec": 128.85055142864172
  },
  "per_subject": {
    "S001": {
      "task_type": "classification",
      "accuracy": 0.6666666666666666,
      "balanced_accuracy": 0.6696428571428572,
      "macro_f1": 0.6666666666666666,
      "weighted_f1": 0.6666666666666666,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.625,
          "recall": 0.7142857142857143,
          "f1-score": 0.6666666666666666,
          "support": 7.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.7142857142857143,
          "recall": 0.625,
          "f1-score": 0.6666666666666666,
          "support": 8.0
        },
        "accuracy": 0.6666666666666666,
        "macro avg": {
          "precision": 0.6696428571428572,
          "recall": 0.6696428571428572,
          "f1-score": 0.6666666666666666,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.6726190476190477,
          "recall": 0.6666666666666666,
          "f1-score": 0.6666666666666666,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          5,
          2
        ],
        [
          3,
          5
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S002": {
      "task_type": "classification",
      "accuracy": 0.7333333333333333,
      "balanced_accuracy": 0.75,
      "macro_f1": 0.7222222222222222,
      "weighted_f1": 0.7185185185185186,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.6363636363636364,
          "recall": 1.0,
          "f1-score": 0.7777777777777778,
          "support": 7.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 1.0,
          "recall": 0.5,
          "f1-score": 0.6666666666666666,
          "support": 8.0
        },
        "accuracy": 0.7333333333333333,
        "macro avg": {
          "precision": 0.8181818181818181,
          "recall": 0.75,
          "f1-score": 0.7222222222222222,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.8303030303030302,
          "recall": 0.7333333333333333,
          "f1-score": 0.7185185185185186,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          7,
          0
        ],
        [
          4,
          4
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S003": {
      "task_type": "classification",
      "accuracy": 0.4,
      "balanced_accuracy": 0.4107142857142857,
      "macro_f1": 0.3891402714932127,
      "weighted_f1": 0.38371040723981903,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.4,
          "recall": 0.5714285714285714,
          "f1-score": 0.47058823529411764,
          "support": 7.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.4,
          "recall": 0.25,
          "f1-score": 0.3076923076923077,
          "support": 8.0
        },
        "accuracy": 0.4,
        "macro avg": {
          "precision": 0.4,
          "recall": 0.4107142857142857,
          "f1-score": 0.3891402714932127,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.4,
          "recall": 0.4,
          "f1-score": 0.38371040723981903,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          4,
          3
        ],
        [
          6,
          2
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S004": {
      "task_type": "classification",
      "accuracy": 0.4,
      "balanced_accuracy": 0.4107142857142857,
      "macro_f1": 0.3891402714932127,
      "weighted_f1": 0.38371040723981903,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.4,
          "recall": 0.25,
          "f1-score": 0.3076923076923077,
          "support": 8.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.4,
          "recall": 0.5714285714285714,
          "f1-score": 0.47058823529411764,
          "support": 7.0
        },
        "accuracy": 0.4,
        "macro avg": {
          "precision": 0.4,
          "recall": 0.4107142857142857,
          "f1-score": 0.3891402714932127,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.4,
          "recall": 0.4,
          "f1-score": 0.38371040723981903,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          2,
          6
        ],
        [
          3,
          4
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S005": {
      "task_type": "classification",
      "accuracy": 0.4,
      "balanced_accuracy": 0.375,
      "macro_f1": 0.2857142857142857,
      "weighted_f1": 0.30476190476190473,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.0,
          "recall": 0.0,
          "f1-score": 0.0,
          "support": 7.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.46153846153846156,
          "recall": 0.75,
          "f1-score": 0.5714285714285714,
          "support": 8.0
        },
        "accuracy": 0.4,
        "macro avg": {
          "precision": 0.23076923076923078,
          "recall": 0.375,
          "f1-score": 0.2857142857142857,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.24615384615384617,
          "recall": 0.4,
          "f1-score": 0.30476190476190473,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          0,
          7
        ],
        [
          2,
          6
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S006": {
      "task_type": "classification",
      "accuracy": 0.5333333333333333,
      "balanced_accuracy": 0.5178571428571428,
      "macro_f1": 0.49760765550239233,
      "weighted_f1": 0.5065390749601275,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5454545454545454,
          "recall": 0.75,
          "f1-score": 0.631578947368421,
          "support": 16.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.5,
          "recall": 0.2857142857142857,
          "f1-score": 0.36363636363636365,
          "support": 14.0
        },
        "accuracy": 0.5333333333333333,
        "macro avg": {
          "precision": 0.5227272727272727,
          "recall": 0.5178571428571428,
          "f1-score": 0.49760765550239233,
          "support": 30.0
        },
        "weighted avg": {
          "precision": 0.5242424242424242,
          "recall": 0.5333333333333333,
          "f1-score": 0.5065390749601275,
          "support": 30.0
        }
      },
      "confusion_matrix": [
        [
          12,
          4
        ],
        [
          10,
          4
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S008": {
      "task_type": "classification",
      "accuracy": 0.6666666666666666,
      "balanced_accuracy": 0.6875,
      "macro_f1": 0.6411483253588517,
      "weighted_f1": 0.634768740031898,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5833333333333334,
          "recall": 1.0,
          "f1-score": 0.7368421052631579,
          "support": 7.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 1.0,
          "recall": 0.375,
          "f1-score": 0.5454545454545454,
          "support": 8.0
        },
        "accuracy": 0.6666666666666666,
        "macro avg": {
          "precision": 0.7916666666666667,
          "recall": 0.6875,
          "f1-score": 0.6411483253588517,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.8055555555555556,
          "recall": 0.6666666666666666,
          "f1-score": 0.634768740031898,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          7,
          0
        ],
        [
          5,
          3
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S009": {
      "task_type": "classification",
      "accuracy": 0.5333333333333333,
      "balanced_accuracy": 0.5,
      "macro_f1": 0.34782608695652173,
      "weighted_f1": 0.3710144927536232,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5333333333333333,
          "recall": 1.0,
          "f1-score": 0.6956521739130435,
          "support": 8.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.0,
          "recall": 0.0,
          "f1-score": 0.0,
          "support": 7.0
        },
        "accuracy": 0.5333333333333333,
        "macro avg": {
          "precision": 0.26666666666666666,
          "recall": 0.5,
          "f1-score": 0.34782608695652173,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.28444444444444444,
          "recall": 0.5333333333333333,
          "f1-score": 0.3710144927536232,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          8,
          0
        ],
        [
          7,
          0
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S010": {
      "task_type": "classification",
      "accuracy": 0.5,
      "balanced_accuracy": 0.4910714285714286,
      "macro_f1": 0.4857142857142857,
      "weighted_f1": 0.49142857142857144,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5263157894736842,
          "recall": 0.625,
          "f1-score": 0.5714285714285714,
          "support": 16.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.45454545454545453,
          "recall": 0.35714285714285715,
          "f1-score": 0.4,
          "support": 14.0
        },
        "accuracy": 0.5,
        "macro avg": {
          "precision": 0.49043062200956933,
          "recall": 0.4910714285714286,
          "f1-score": 0.4857142857142857,
          "support": 30.0
        },
        "weighted avg": {
          "precision": 0.492822966507177,
          "recall": 0.5,
          "f1-score": 0.49142857142857144,
          "support": 30.0
        }
      },
      "confusion_matrix": [
        [
          10,
          6
        ],
        [
          9,
          5
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    }
  },
  "per_run": {
    "12": {
      "task_type": "classification",
      "accuracy": 0.5222222222222223,
      "balanced_accuracy": 0.5212450592885376,
      "macro_f1": 0.5207430340557275,
      "weighted_f1": 0.5213347093223255,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5306122448979592,
          "recall": 0.5652173913043478,
          "f1-score": 0.5473684210526316,
          "support": 46.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.5121951219512195,
          "recall": 0.4772727272727273,
          "f1-score": 0.49411764705882355,
          "support": 44.0
        },
        "accuracy": 0.5222222222222223,
        "macro avg": {
          "precision": 0.5214036834245894,
          "recall": 0.5212450592885376,
          "f1-score": 0.5207430340557275,
          "support": 90.0
        },
        "weighted avg": {
          "precision": 0.5216083181239975,
          "recall": 0.5222222222222223,
          "f1-score": 0.5213347093223255,
          "support": 90.0
        }
      },
      "confusion_matrix": [
        [
          26,
          20
        ],
        [
          23,
          21
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "4": {
      "task_type": "classification",
      "accuracy": 0.6333333333333333,
      "balanced_accuracy": 0.6333333333333333,
      "macro_f1": 0.5970695970695971,
      "weighted_f1": 0.5970695970695971,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5833333333333334,
          "recall": 0.9333333333333333,
          "f1-score": 0.717948717948718,
          "support": 15.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.8333333333333334,
          "recall": 0.3333333333333333,
          "f1-score": 0.47619047619047616,
          "support": 15.0
        },
        "accuracy": 0.6333333333333333,
        "macro avg": {
          "precision": 0.7083333333333334,
          "recall": 0.6333333333333333,
          "f1-score": 0.5970695970695971,
          "support": 30.0
        },
        "weighted avg": {
          "precision": 0.7083333333333334,
          "recall": 0.6333333333333333,
          "f1-score": 0.5970695970695971,
          "support": 30.0
        }
      },
      "confusion_matrix": [
        [
          14,
          1
        ],
        [
          10,
          5
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "8": {
      "task_type": "classification",
      "accuracy": 0.4888888888888889,
      "balanced_accuracy": 0.49308300395256915,
      "macro_f1": 0.4722080571137175,
      "weighted_f1": 0.47012295314182106,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.4838709677419355,
          "recall": 0.6818181818181818,
          "f1-score": 0.5660377358490566,
          "support": 22.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.5,
          "recall": 0.30434782608695654,
          "f1-score": 0.3783783783783784,
          "support": 23.0
        },
        "accuracy": 0.4888888888888889,
        "macro avg": {
          "precision": 0.49193548387096775,
          "recall": 0.49308300395256915,
          "f1-score": 0.4722080571137175,
          "support": 45.0
        },
        "weighted avg": {
          "precision": 0.4921146953405018,
          "recall": 0.4888888888888889,
          "f1-score": 0.47012295314182106,
          "support": 45.0
        }
      },
      "confusion_matrix": [
        [
          15,
          7
        ],
        [
          16,
          7
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    }
  },
  "bootstrap_confidence_intervals": {
    "available": true,
    "n_bootstrap": 500,
    "seed": 42,
    "accuracy": {
      "mean": 0.5337,
      "lower_95": 0.4545,
      "upper_95": 0.6121
    },
    "balanced_accuracy": {
      "mean": 0.5333,
      "lower_95": 0.4627,
      "upper_95": 0.6141
    },
    "macro_f1": {
      "mean": 0.5239,
      "lower_95": 0.4454,
      "upper_95": 0.6046
    }
  }
}
```