# Model Card: physionet_eegbci_medium_csp_logreg_csp8

## Intended Use
Offline event-locked motor imagery task-label classification.

## Not Intended Use
Not for thought reading, clinical diagnosis, unrestricted mental-state inference, or direct closed-loop control.

## Limitations
Small public EEG subset; CSP is fit inside train splits. Metrics are benchmark evidence only and do not establish clinical validity.

## Metrics
```json
{
  "task_type": "classification",
  "accuracy": 0.5212121212121212,
  "balanced_accuracy": 0.520129297678519,
  "macro_f1": 0.5048433048433049,
  "weighted_f1": 0.5053889320555986,
  "per_class": {
    "LEFT_HAND_IMAGERY": {
      "precision": 0.5178571428571429,
      "recall": 0.6987951807228916,
      "f1-score": 0.5948717948717949,
      "support": 83.0
    },
    "RIGHT_HAND_IMAGERY": {
      "precision": 0.5283018867924528,
      "recall": 0.34146341463414637,
      "f1-score": 0.4148148148148148,
      "support": 82.0
    },
    "accuracy": 0.5212121212121212,
    "macro avg": {
      "precision": 0.5230795148247979,
      "recall": 0.520129297678519,
      "f1-score": 0.5048433048433049,
      "support": 165.0
    },
    "weighted avg": {
      "precision": 0.5230478640855999,
      "recall": 0.5212121212121212,
      "f1-score": 0.5053889320555986,
      "support": 165.0
    }
  },
  "confusion_matrix": [
    [
      58,
      25
    ],
    [
      54,
      28
    ]
  ],
  "class_labels": [
    "LEFT_HAND_IMAGERY",
    "RIGHT_HAND_IMAGERY"
  ],
  "calibration": {
    "available": true,
    "ece": 0.18034842357640732,
    "mce": 0.3625012250890631,
    "reliability_bins": [
      {
        "lo": 0.0,
        "hi": 0.1,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.1,
        "hi": 0.2,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.2,
        "hi": 0.30000000000000004,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.30000000000000004,
        "hi": 0.4,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.4,
        "hi": 0.5,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.5,
        "hi": 0.6,
        "count": 40,
        "accuracy": 0.35,
        "confidence": 0.5444719841486038
      },
      {
        "lo": 0.6000000000000001,
        "hi": 0.7000000000000001,
        "count": 45,
        "accuracy": 0.4444444444444444,
        "confidence": 0.6554589973167301
      },
      {
        "lo": 0.7000000000000001,
        "hi": 0.8,
        "count": 41,
        "accuracy": 0.7317073170731707,
        "confidence": 0.7462270672954745
      },
      {
        "lo": 0.8,
        "hi": 0.9,
        "count": 29,
        "accuracy": 0.4827586206896552,
        "confidence": 0.8452598457787183
      },
      {
        "lo": 0.9,
        "hi": 1.0,
        "count": 10,
        "accuracy": 0.8,
        "confidence": 0.9375110358212915
      }
    ],
    "brier_score": 0.26964462888424623
  },
  "auroc": 0.5969732588892154,
  "split_strategy": "group_run",
  "split_warnings": [],
  "latency": {
    "mean_inference_latency_ms": 0.3204889455060545,
    "p95_inference_latency_ms": 0.3792139996221521,
    "p99_inference_latency_ms": 0.4785511195950663,
    "throughput_samples_per_sec": 3120.232426179294
  },
  "per_subject": {
    "S001": {
      "task_type": "classification",
      "accuracy": 0.4,
      "balanced_accuracy": 0.41964285714285715,
      "macro_f1": 0.354066985645933,
      "weighted_f1": 0.34258373205741627,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.4166666666666667,
          "recall": 0.7142857142857143,
          "f1-score": 0.5263157894736842,
          "support": 7.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.3333333333333333,
          "recall": 0.125,
          "f1-score": 0.18181818181818182,
          "support": 8.0
        },
        "accuracy": 0.4,
        "macro avg": {
          "precision": 0.375,
          "recall": 0.41964285714285715,
          "f1-score": 0.354066985645933,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.3722222222222223,
          "recall": 0.4,
          "f1-score": 0.34258373205741627,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          5,
          2
        ],
        [
          7,
          1
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S002": {
      "task_type": "classification",
      "accuracy": 0.8,
      "balanced_accuracy": 0.7857142857142857,
      "macro_f1": 0.784688995215311,
      "weighted_f1": 0.7885167464114832,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 1.0,
          "recall": 0.5714285714285714,
          "f1-score": 0.7272727272727273,
          "support": 7.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.7272727272727273,
          "recall": 1.0,
          "f1-score": 0.8421052631578947,
          "support": 8.0
        },
        "accuracy": 0.8,
        "macro avg": {
          "precision": 0.8636363636363636,
          "recall": 0.7857142857142857,
          "f1-score": 0.784688995215311,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.8545454545454546,
          "recall": 0.8,
          "f1-score": 0.7885167464114832,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          4,
          3
        ],
        [
          0,
          8
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S003": {
      "task_type": "classification",
      "accuracy": 0.3333333333333333,
      "balanced_accuracy": 0.3214285714285714,
      "macro_f1": 0.3055555555555555,
      "weighted_f1": 0.31481481481481477,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.2,
          "recall": 0.14285714285714285,
          "f1-score": 0.16666666666666666,
          "support": 7.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.4,
          "recall": 0.5,
          "f1-score": 0.4444444444444444,
          "support": 8.0
        },
        "accuracy": 0.3333333333333333,
        "macro avg": {
          "precision": 0.30000000000000004,
          "recall": 0.3214285714285714,
          "f1-score": 0.3055555555555555,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.3066666666666667,
          "recall": 0.3333333333333333,
          "f1-score": 0.31481481481481477,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          1,
          6
        ],
        [
          4,
          4
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S004": {
      "task_type": "classification",
      "accuracy": 0.5333333333333333,
      "balanced_accuracy": 0.5178571428571428,
      "macro_f1": 0.49760765550239233,
      "weighted_f1": 0.5065390749601275,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5454545454545454,
          "recall": 0.75,
          "f1-score": 0.631578947368421,
          "support": 8.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.5,
          "recall": 0.2857142857142857,
          "f1-score": 0.36363636363636365,
          "support": 7.0
        },
        "accuracy": 0.5333333333333333,
        "macro avg": {
          "precision": 0.5227272727272727,
          "recall": 0.5178571428571428,
          "f1-score": 0.49760765550239233,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.5242424242424242,
          "recall": 0.5333333333333333,
          "f1-score": 0.5065390749601275,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          6,
          2
        ],
        [
          5,
          2
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S005": {
      "task_type": "classification",
      "accuracy": 0.4666666666666667,
      "balanced_accuracy": 0.4375,
      "macro_f1": 0.3181818181818182,
      "weighted_f1": 0.3393939393939394,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.0,
          "recall": 0.0,
          "f1-score": 0.0,
          "support": 7.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.5,
          "recall": 0.875,
          "f1-score": 0.6363636363636364,
          "support": 8.0
        },
        "accuracy": 0.4666666666666667,
        "macro avg": {
          "precision": 0.25,
          "recall": 0.4375,
          "f1-score": 0.3181818181818182,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.26666666666666666,
          "recall": 0.4666666666666667,
          "f1-score": 0.3393939393939394,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          0,
          7
        ],
        [
          1,
          7
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S006": {
      "task_type": "classification",
      "accuracy": 0.5333333333333333,
      "balanced_accuracy": 0.5,
      "macro_f1": 0.34782608695652173,
      "weighted_f1": 0.3710144927536232,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5333333333333333,
          "recall": 1.0,
          "f1-score": 0.6956521739130435,
          "support": 16.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.0,
          "recall": 0.0,
          "f1-score": 0.0,
          "support": 14.0
        },
        "accuracy": 0.5333333333333333,
        "macro avg": {
          "precision": 0.26666666666666666,
          "recall": 0.5,
          "f1-score": 0.34782608695652173,
          "support": 30.0
        },
        "weighted avg": {
          "precision": 0.28444444444444444,
          "recall": 0.5333333333333333,
          "f1-score": 0.3710144927536232,
          "support": 30.0
        }
      },
      "confusion_matrix": [
        [
          16,
          0
        ],
        [
          14,
          0
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S008": {
      "task_type": "classification",
      "accuracy": 0.6,
      "balanced_accuracy": 0.6071428571428572,
      "macro_f1": 0.5982142857142857,
      "weighted_f1": 0.5964285714285714,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5555555555555556,
          "recall": 0.7142857142857143,
          "f1-score": 0.625,
          "support": 7.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.6666666666666666,
          "recall": 0.5,
          "f1-score": 0.5714285714285714,
          "support": 8.0
        },
        "accuracy": 0.6,
        "macro avg": {
          "precision": 0.6111111111111112,
          "recall": 0.6071428571428572,
          "f1-score": 0.5982142857142857,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.6148148148148148,
          "recall": 0.6,
          "f1-score": 0.5964285714285714,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          5,
          2
        ],
        [
          4,
          4
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S009": {
      "task_type": "classification",
      "accuracy": 0.5333333333333333,
      "balanced_accuracy": 0.5,
      "macro_f1": 0.34782608695652173,
      "weighted_f1": 0.3710144927536232,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5333333333333333,
          "recall": 1.0,
          "f1-score": 0.6956521739130435,
          "support": 8.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.0,
          "recall": 0.0,
          "f1-score": 0.0,
          "support": 7.0
        },
        "accuracy": 0.5333333333333333,
        "macro avg": {
          "precision": 0.26666666666666666,
          "recall": 0.5,
          "f1-score": 0.34782608695652173,
          "support": 15.0
        },
        "weighted avg": {
          "precision": 0.28444444444444444,
          "recall": 0.5333333333333333,
          "f1-score": 0.3710144927536232,
          "support": 15.0
        }
      },
      "confusion_matrix": [
        [
          8,
          0
        ],
        [
          7,
          0
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S010": {
      "task_type": "classification",
      "accuracy": 0.5,
      "balanced_accuracy": 0.4776785714285714,
      "macro_f1": 0.42233632862644416,
      "weighted_f1": 0.43645699614890887,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.52,
          "recall": 0.8125,
          "f1-score": 0.6341463414634146,
          "support": 16.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.4,
          "recall": 0.14285714285714285,
          "f1-score": 0.21052631578947367,
          "support": 14.0
        },
        "accuracy": 0.5,
        "macro avg": {
          "precision": 0.46,
          "recall": 0.4776785714285714,
          "f1-score": 0.42233632862644416,
          "support": 30.0
        },
        "weighted avg": {
          "precision": 0.4640000000000001,
          "recall": 0.5,
          "f1-score": 0.43645699614890887,
          "support": 30.0
        }
      },
      "confusion_matrix": [
        [
          13,
          3
        ],
        [
          12,
          2
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    }
  },
  "per_run": {
    "12": {
      "task_type": "classification",
      "accuracy": 0.5,
      "balanced_accuracy": 0.4940711462450593,
      "macro_f1": 0.4581939799331104,
      "weighted_f1": 0.46153846153846156,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5072463768115942,
          "recall": 0.7608695652173914,
          "f1-score": 0.6086956521739131,
          "support": 46.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.47619047619047616,
          "recall": 0.22727272727272727,
          "f1-score": 0.3076923076923077,
          "support": 44.0
        },
        "accuracy": 0.5,
        "macro avg": {
          "precision": 0.4917184265010352,
          "recall": 0.4940711462450593,
          "f1-score": 0.4581939799331104,
          "support": 90.0
        },
        "weighted avg": {
          "precision": 0.49206349206349215,
          "recall": 0.5,
          "f1-score": 0.46153846153846156,
          "support": 90.0
        }
      },
      "confusion_matrix": [
        [
          35,
          11
        ],
        [
          34,
          10
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "4": {
      "task_type": "classification",
      "accuracy": 0.6333333333333333,
      "balanced_accuracy": 0.6333333333333333,
      "macro_f1": 0.6329254727474972,
      "weighted_f1": 0.6329254727474972,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.6428571428571429,
          "recall": 0.6,
          "f1-score": 0.6206896551724138,
          "support": 15.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.625,
          "recall": 0.6666666666666666,
          "f1-score": 0.6451612903225806,
          "support": 15.0
        },
        "accuracy": 0.6333333333333333,
        "macro avg": {
          "precision": 0.6339285714285714,
          "recall": 0.6333333333333333,
          "f1-score": 0.6329254727474972,
          "support": 30.0
        },
        "weighted avg": {
          "precision": 0.6339285714285715,
          "recall": 0.6333333333333333,
          "f1-score": 0.6329254727474972,
          "support": 30.0
        }
      },
      "confusion_matrix": [
        [
          9,
          6
        ],
        [
          5,
          10
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "8": {
      "task_type": "classification",
      "accuracy": 0.4888888888888889,
      "balanced_accuracy": 0.49209486166007904,
      "macro_f1": 0.4796380090497738,
      "weighted_f1": 0.4780961957432546,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.4827586206896552,
          "recall": 0.6363636363636364,
          "f1-score": 0.5490196078431373,
          "support": 22.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.5,
          "recall": 0.34782608695652173,
          "f1-score": 0.41025641025641024,
          "support": 23.0
        },
        "accuracy": 0.4888888888888889,
        "macro avg": {
          "precision": 0.4913793103448276,
          "recall": 0.49209486166007904,
          "f1-score": 0.4796380090497738,
          "support": 45.0
        },
        "weighted avg": {
          "precision": 0.49157088122605364,
          "recall": 0.4888888888888889,
          "f1-score": 0.4780961957432546,
          "support": 45.0
        }
      },
      "confusion_matrix": [
        [
          14,
          8
        ],
        [
          15,
          8
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    }
  },
  "bootstrap_confidence_intervals": {
    "available": true,
    "n_bootstrap": 500,
    "seed": 42,
    "accuracy": {
      "mean": 0.5196,
      "lower_95": 0.4424,
      "upper_95": 0.5939
    },
    "balanced_accuracy": {
      "mean": 0.519,
      "lower_95": 0.4441,
      "upper_95": 0.5878
    },
    "macro_f1": {
      "mean": 0.5019,
      "lower_95": 0.4199,
      "upper_95": 0.5757
    }
  }
}
```