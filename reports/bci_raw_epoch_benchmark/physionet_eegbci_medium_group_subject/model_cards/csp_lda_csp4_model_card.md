# Model Card: physionet_eegbci_medium_group_subject_csp_lda_csp4

## Intended Use
Offline event-locked motor imagery task-label classification.

## Not Intended Use
Not for thought reading, clinical diagnosis, unrestricted mental-state inference, or direct closed-loop control.

## Limitations
Small public EEG subset; CSP is fit inside train splits. Metrics are benchmark evidence only and do not establish clinical validity.

## Metrics
```json
{
  "task_type": "classification",
  "accuracy": 0.4888888888888889,
  "balanced_accuracy": 0.4804552201880258,
  "macro_f1": 0.4650471637162424,
  "weighted_f1": 0.4700664742789048,
  "per_class": {
    "LEFT_HAND_IMAGERY": {
      "precision": 0.5080645161290323,
      "recall": 0.6702127659574468,
      "f1-score": 0.5779816513761468,
      "support": 94.0
    },
    "RIGHT_HAND_IMAGERY": {
      "precision": 0.44642857142857145,
      "recall": 0.29069767441860467,
      "f1-score": 0.352112676056338,
      "support": 86.0
    },
    "accuracy": 0.4888888888888889,
    "macro avg": {
      "precision": 0.4772465437788018,
      "recall": 0.4804552201880258,
      "f1-score": 0.4650471637162424,
      "support": 180.0
    },
    "weighted avg": {
      "precision": 0.4786162314388121,
      "recall": 0.4888888888888889,
      "f1-score": 0.4700664742789048,
      "support": 180.0
    }
  },
  "confusion_matrix": [
    [
      63,
      31
    ],
    [
      61,
      25
    ]
  ],
  "class_labels": [
    "LEFT_HAND_IMAGERY",
    "RIGHT_HAND_IMAGERY"
  ],
  "calibration": {
    "available": true,
    "ece": 0.3247413884057899,
    "mce": 0.3803046806773632,
    "reliability_bins": [
      {
        "lo": 0.0,
        "hi": 0.1,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.1,
        "hi": 0.2,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.2,
        "hi": 0.30000000000000004,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.30000000000000004,
        "hi": 0.4,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.4,
        "hi": 0.5,
        "count": 0,
        "accuracy": null,
        "confidence": null
      },
      {
        "lo": 0.5,
        "hi": 0.6,
        "count": 12,
        "accuracy": 0.4166666666666667,
        "confidence": 0.5355522357008978
      },
      {
        "lo": 0.6000000000000001,
        "hi": 0.7000000000000001,
        "count": 18,
        "accuracy": 0.3333333333333333,
        "confidence": 0.6644870857210539
      },
      {
        "lo": 0.7000000000000001,
        "hi": 0.8,
        "count": 35,
        "accuracy": 0.5142857142857142,
        "confidence": 0.7596008159128337
      },
      {
        "lo": 0.8,
        "hi": 0.9,
        "count": 75,
        "accuracy": 0.49333333333333335,
        "confidence": 0.8569045301014498
      },
      {
        "lo": 0.9,
        "hi": 1.0,
        "count": 40,
        "accuracy": 0.55,
        "confidence": 0.9303046806773633
      }
    ],
    "brier_score": 0.3584451756097309
  },
  "auroc": 0.5070509648688768,
  "split_strategy": "group_subject",
  "split_warnings": [],
  "latency": {
    "mean_inference_latency_ms": 0.30502806108011604,
    "p95_inference_latency_ms": 0.3645779502221554,
    "p99_inference_latency_ms": 0.47287443076129465,
    "throughput_samples_per_sec": 3278.386901385275
  },
  "per_subject": {
    "S001": {
      "task_type": "classification",
      "accuracy": 0.5111111111111111,
      "balanced_accuracy": 0.5,
      "macro_f1": 0.3382352941176471,
      "weighted_f1": 0.34575163398692815,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5111111111111111,
          "recall": 1.0,
          "f1-score": 0.6764705882352942,
          "support": 23.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.0,
          "recall": 0.0,
          "f1-score": 0.0,
          "support": 22.0
        },
        "accuracy": 0.5111111111111111,
        "macro avg": {
          "precision": 0.25555555555555554,
          "recall": 0.5,
          "f1-score": 0.3382352941176471,
          "support": 45.0
        },
        "weighted avg": {
          "precision": 0.26123456790123456,
          "recall": 0.5111111111111111,
          "f1-score": 0.34575163398692815,
          "support": 45.0
        }
      },
      "confusion_matrix": [
        [
          23,
          0
        ],
        [
          22,
          0
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S002": {
      "task_type": "classification",
      "accuracy": 0.5111111111111111,
      "balanced_accuracy": 0.5,
      "macro_f1": 0.3382352941176471,
      "weighted_f1": 0.34575163398692815,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5111111111111111,
          "recall": 1.0,
          "f1-score": 0.6764705882352942,
          "support": 23.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.0,
          "recall": 0.0,
          "f1-score": 0.0,
          "support": 22.0
        },
        "accuracy": 0.5111111111111111,
        "macro avg": {
          "precision": 0.25555555555555554,
          "recall": 0.5,
          "f1-score": 0.3382352941176471,
          "support": 45.0
        },
        "weighted avg": {
          "precision": 0.26123456790123456,
          "recall": 0.5111111111111111,
          "f1-score": 0.34575163398692815,
          "support": 45.0
        }
      },
      "confusion_matrix": [
        [
          23,
          0
        ],
        [
          22,
          0
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S006": {
      "task_type": "classification",
      "accuracy": 0.4666666666666667,
      "balanced_accuracy": 0.4494047619047619,
      "macro_f1": 0.41810344827586204,
      "weighted_f1": 0.4293103448275862,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5,
          "recall": 0.7083333333333334,
          "f1-score": 0.5862068965517241,
          "support": 24.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.36363636363636365,
          "recall": 0.19047619047619047,
          "f1-score": 0.25,
          "support": 21.0
        },
        "accuracy": 0.4666666666666667,
        "macro avg": {
          "precision": 0.4318181818181818,
          "recall": 0.4494047619047619,
          "f1-score": 0.41810344827586204,
          "support": 45.0
        },
        "weighted avg": {
          "precision": 0.4363636363636364,
          "recall": 0.4666666666666667,
          "f1-score": 0.4293103448275862,
          "support": 45.0
        }
      },
      "confusion_matrix": [
        [
          17,
          7
        ],
        [
          17,
          4
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "S009": {
      "task_type": "classification",
      "accuracy": 0.4666666666666667,
      "balanced_accuracy": 0.5,
      "macro_f1": 0.3181818181818182,
      "weighted_f1": 0.29696969696969694,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.0,
          "recall": 0.0,
          "f1-score": 0.0,
          "support": 24.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.4666666666666667,
          "recall": 1.0,
          "f1-score": 0.6363636363636364,
          "support": 21.0
        },
        "accuracy": 0.4666666666666667,
        "macro avg": {
          "precision": 0.23333333333333334,
          "recall": 0.5,
          "f1-score": 0.3181818181818182,
          "support": 45.0
        },
        "weighted avg": {
          "precision": 0.2177777777777778,
          "recall": 0.4666666666666667,
          "f1-score": 0.29696969696969694,
          "support": 45.0
        }
      },
      "confusion_matrix": [
        [
          0,
          24
        ],
        [
          0,
          21
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    }
  },
  "per_run": {
    "12": {
      "task_type": "classification",
      "accuracy": 0.48333333333333334,
      "balanced_accuracy": 0.4799777530589544,
      "macro_f1": 0.4762038862292312,
      "weighted_f1": 0.47824087111611757,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5,
          "recall": 0.5806451612903226,
          "f1-score": 0.5373134328358209,
          "support": 31.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.4583333333333333,
          "recall": 0.3793103448275862,
          "f1-score": 0.41509433962264153,
          "support": 29.0
        },
        "accuracy": 0.48333333333333334,
        "macro avg": {
          "precision": 0.47916666666666663,
          "recall": 0.4799777530589544,
          "f1-score": 0.4762038862292312,
          "support": 60.0
        },
        "weighted avg": {
          "precision": 0.47986111111111107,
          "recall": 0.48333333333333334,
          "f1-score": 0.47824087111611757,
          "support": 60.0
        }
      },
      "confusion_matrix": [
        [
          18,
          13
        ],
        [
          18,
          11
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "4": {
      "task_type": "classification",
      "accuracy": 0.48333333333333334,
      "balanced_accuracy": 0.4755283648498332,
      "macro_f1": 0.4488888888888889,
      "weighted_f1": 0.4534814814814815,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5,
          "recall": 0.7096774193548387,
          "f1-score": 0.5866666666666667,
          "support": 31.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.4375,
          "recall": 0.2413793103448276,
          "f1-score": 0.3111111111111111,
          "support": 29.0
        },
        "accuracy": 0.48333333333333334,
        "macro avg": {
          "precision": 0.46875,
          "recall": 0.4755283648498332,
          "f1-score": 0.4488888888888889,
          "support": 60.0
        },
        "weighted avg": {
          "precision": 0.46979166666666666,
          "recall": 0.48333333333333334,
          "f1-score": 0.4534814814814815,
          "support": 60.0
        }
      },
      "confusion_matrix": [
        [
          22,
          9
        ],
        [
          22,
          7
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    },
    "8": {
      "task_type": "classification",
      "accuracy": 0.5,
      "balanced_accuracy": 0.484375,
      "macro_f1": 0.46172248803827753,
      "weighted_f1": 0.47129186602870815,
      "per_class": {
        "LEFT_HAND_IMAGERY": {
          "precision": 0.5227272727272727,
          "recall": 0.71875,
          "f1-score": 0.6052631578947368,
          "support": 32.0
        },
        "RIGHT_HAND_IMAGERY": {
          "precision": 0.4375,
          "recall": 0.25,
          "f1-score": 0.3181818181818182,
          "support": 28.0
        },
        "accuracy": 0.5,
        "macro avg": {
          "precision": 0.48011363636363635,
          "recall": 0.484375,
          "f1-score": 0.46172248803827753,
          "support": 60.0
        },
        "weighted avg": {
          "precision": 0.48295454545454547,
          "recall": 0.5,
          "f1-score": 0.47129186602870815,
          "support": 60.0
        }
      },
      "confusion_matrix": [
        [
          23,
          9
        ],
        [
          21,
          7
        ]
      ],
      "class_labels": [
        "LEFT_HAND_IMAGERY",
        "RIGHT_HAND_IMAGERY"
      ],
      "calibration": {
        "available": false
      }
    }
  },
  "bootstrap_confidence_intervals": {
    "available": true,
    "n_bootstrap": 500,
    "seed": 42,
    "accuracy": {
      "mean": 0.4887,
      "lower_95": 0.4167,
      "upper_95": 0.5667
    },
    "balanced_accuracy": {
      "mean": 0.4816,
      "lower_95": 0.4079,
      "upper_95": 0.5546
    },
    "macro_f1": {
      "mean": 0.4643,
      "lower_95": 0.3884,
      "upper_95": 0.5407
    }
  }
}
```